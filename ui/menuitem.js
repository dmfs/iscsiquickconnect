...
export var EditableMenuItem = GObject.registerClass(
  class IscsiEditableMenuItem extends PopupMenu.PopupBaseMenuItem {
    _init(...actions) {
      ...
      const vertical_layout = new St.BoxLayout({
        vertical: true,
        x_expand: true,
      });
      const inner_layout = new St.BoxLayout({
        vertical: false,
        x_expand: true,
      });
      ...
      actions
        .map((action) => new ActionButton(action, () => entry.text))
    ...
    set_error(error) {
      this.error.set_child(
        new St.Label({
          text: error,
          x_expand: true,
          style: "font-weight:bold; padding-top: 4pt",
        }),
      );
    }
  },
